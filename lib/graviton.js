...
var preloadScript = __dirname + "/preload.js";

function _prepareScript(fn) {
    var args = JSON.stringify(Array.prototype.slice.call(arguments).slice(1)).slice(1, -1);
    var script = String(fn);
    return `
            (function () {
                try {
                    (${script})(${args});
...
// todo: use async
function _genTmpPreloadScript(fns) {
    var args = Array.prototype.slice.call(arguments);

    var scripts = [];
    if (typeof args[0] == "function") {
        scripts.push(_prepareScript.apply(this, args));
    } else {
        for (var i = 0; i < args.length; i++) {
            if (_.isArray(args[i]) && _.isFunction(args[i][0])) {
                scripts.push(_prepareScript.apply(this, args[i]));
            }
        }
    }

    var name = tmp.tmpNameSync();
    fs.writeFileSync(name, scripts.join("\r\n"));
    return name;
}
...
class Graviton {
     ...
     * @returns {Promise}
     */
    static new(options, preload) {
        return new Promise((resolve, reject)=> {
            if (isAppReady) {
                resolve(new Graviton(options, preload));
            } else {
                app.on("ready", ()=> {
                    isAppReady = true;
     ...
     * @param preload - function script for preload
     */
    constructor(options, preload) {
        ...
        if (options.proxy) {
            ...
            options.electron.webPreferences.session.setProxy("http=" + proxy.host, ()=> {
                debug("used session proxy %s", options.proxy);
            });
        }

        var preloadArgs = [];

        if (preload && _.isArray(preload) && _.isFunction(preload[0])) {
            preloadArgs.push(preload);
        }

        if (_.isPlainObject(options.navigator)) {
            for (var key in options.navigator) {
                preloadArgs.push([setNavProp, key, options.navigator[key]]);
            }
        }

        this._isTmpPreload = false;
        if (preloadArgs.length) {
            preloadArgs.push([function () {
                var ipc = require('electron').ipcRenderer;
                window.__graviton__ = {};
                window.__graviton__.ipc = ipc;
            }]);
            options.electron.webPreferences.preload = _genTmpPreloadScript.apply(this, preloadArgs);//preload;
            this._isTmpPreload = true;
        } else {
            options.electron.webPreferences.preload = preloadScript;
        }
...
(function javascript () {
    ...
    console.log = log;
            ...
            tm = setTimeout(()=> {
                complete();
                reject(new GravitonError("failed to evaluate script (passed waitTimeout)"));
            }, Graviton.defaults.waitTimeout);
    ...
    wait() {
        ...
        if (typeof args[0] == "number") {
            ...
            return this.wait(Graviton.defaults.waitPreventTimeout).then(()=> {
                return new Promise((resolve, reject)=> {
                    var tm = null;
     ...
     */
        ...
        return new Promise((resolve, reject)=> {
            self.win.once('closed', function () {
                if (self._isTmpPreload) {
                    fs.unlink(self.options.electron.webPreferences.preload);
                }
                resolve();
            });
            self.win.destroy();
